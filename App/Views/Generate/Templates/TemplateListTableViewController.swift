...
class TemplateListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: myBarcodeSegues.EditTemplateSegue, sender: Model.Templates[indexPath.section][indexPath.row + 1])
    }

    // MARK: - Navigation
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == myBarcodeSegues.EditTemplateSegue,
