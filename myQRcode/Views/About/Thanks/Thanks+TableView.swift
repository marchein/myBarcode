...
extension ThanksTableViewController {
    override func numberOfSections(in tableView: UITableView) -> Int {
        return myQRcode.thanksItems.count
    }
    
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        let sectionValues = myQRcode.thanksItems[section]["items"] as? [Thank]
        return sectionValues?.count ?? 0
    }
    
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        return myQRcode.thanksItems[section]["header"] as? String
    }
    
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "thanksCell", for: indexPath)
        let thanksValues = myQRcode.thanksItems[indexPath.section]["items"] as! [Thank]
